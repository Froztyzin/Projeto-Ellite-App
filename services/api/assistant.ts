import { allMembers, invoices, expenses, enrollments } from './database';
...
export const getAiAssistantResponse = async (question: string): Promise<string> => {
    ...
    if (q.includes('aluno')) {
        ...
        return `O número total de alunos (ativos e inativos) é ${allMembers.length}.`;
    }
    ...
    if (q.includes('despesa')) {
        ...
        const total = expenses
            .filter(e => new Date(e.data) >= dateLimit)
    ...
    if (q.includes('plano') && (q.includes('popular') || q.includes('vendido'))) {
        const planCounts = enrollments.reduce((acc, e) => {
            acc[e.plan.nome] = (acc[e.plan.nome] || 0) + 1;
            return acc;
        }, {} as { [key: string]: number });
        
        const mostPopular = Object.entries(planCounts).sort((a, b) => b[1] - a[1])[0];
        if (!mostPopular) return 'Não há planos populares no momento.';
        return `O plano mais popular é o "${mostPopular[0]}" com ${mostPopular[1]} matrículas.`;
    }
