...
const MemberProfile: React.FC = () => {
    ...
    const { data, isLoading, error } = useQuery({
      ...
      queryFn: async () => {
        ...
        if (!memberData) throw new Error("Aluno não encontrado");
    ...
    useEffect(() => {
        if(data?.member) {
            setNotes(data.member.observacoes || '');
    ...
    const handleNotesChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        setIsNotesChanged(e.target.value !== (data?.member?.observacoes || ''));
    }
    ...
    if (error) return <div className="text-center p-10 text-red-500">{(error as Error).message}</div>;
        ...
        <>
            <div className="mb-6">
                <Link to="/members" className="flex items-center text-sm font-medium text-primary-500 hover:text-primary-400">
                    <FaArrowLeft className="mr-2" /> Voltar para Alunos
                </Link>
            </div>
                ...
                <div className="lg:col-span-1 space-y-8">
                    ...
                    {enrollment && (
                     ...
                     <div className="bg-card p-6 rounded-lg border border-slate-700 shadow-sm">
                        ...
                        <textarea value={notes} onChange={handleNotesChange} rows={5} className="w-full p-2 border border-slate-600 bg-slate-700 text-slate-200 rounded-md text-sm focus:ring-primary-500 focus:border-primary-500" placeholder="Adicione anotações sobre o aluno..."/>
                        <button onClick={handleSaveNotes} disabled={!isNotesChanged || updateNotesMutation.isPending} className="mt-3 w-full flex items-center justify-center bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700 transition disabled:bg-slate-500 disabled:cursor-not-allowed">
                            ...
                            <tbody className="bg-card divide-y divide-slate-700">
                                {invoices && invoices.length > 0 ? invoices.map(invoice => (
                                    <tr key={invoice.id}>
                                        ...
                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-slate-300">{formatDateOnly(new Date(invoice.vencimento))}</td>
                                        <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-semibold text-slate-200">{formatCurrency(invoice.valor)}</td>
                                        <td className="px-4 py-4 whitespace-nowrap text-center">{getStatusBadge(invoice.status)}</td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan={4} className="text-center py-10 text-slate-400">Nenhuma fatura encontrada.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
