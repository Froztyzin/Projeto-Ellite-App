...
// Fix: Defined an interface for the dashboard data structure to resolve type errors.
...
const Dashboard: React.FC = () => {
    // Fix: Typed the useQuery hook to ensure 'data' has the correct structure.
    ...
    // Fix: Typed the useQuery hooks for invoices and expenses.
    const { data: allInvoices = [] } = useQuery<Invoice[]>({ queryKey: ['invoices'], queryFn: getInvoices, enabled: !isLoading });
    const { data: allExpenses = [] } = useQuery<Expense[]>({ queryKey: ['expenses'], queryFn: getExpenses, enabled: !isLoading });
